...
//  TIM_Cmd(TIM3, ENABLE);
  ...
  while (1)
  {
	...
	if(RxRingBuf.length > 1) {
		uint32_t i;
		uint32_t ntosend = RxRingBuf.length;
		uint16_t b1;
		uint16_t b2;


		b1 = BufferRead(&RxRingBuf);
		b2 = BufferRead(&RxRingBuf);
		//for(i = 0; i < ntosend;i++)
		//{
			...
			if((b1 & 0xf0) == 0xf0 )
			{
				...
				if(ds == 0x00 || ds == 0x01) Sevenseg_Send(ds,b2);
			}
