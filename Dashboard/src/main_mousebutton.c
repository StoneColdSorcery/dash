...
/** @addtogroup STM32F401-Discovery_Demo
  * @{
  */
...
#define TESTRESULT_ADDRESS         0x0803FFFC
...
__IO int8_t X_Offset, Y_Offset;
__IO uint16_t EchoCnt = 0x00;
__IO uint8_t Echo_InProg = 0x00;
__IO uint16_t Echo_Result = 0x00;
__IO uint16_t Echo_Result_Flag = 0x00;
__IO uint16_t Echo_WaitCount = 0x00;
uint8_t Buffer_X[6];
uint8_t Buffer_Y[6];
...
void Brad_Init()
{
	  ...
	  TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
	   ...
	   // initialize echo external interrupt
...
void TIM3_IRQHandler(void)
{
	...
	if(Echo_WaitCount > 999) {

		if(Echo_InProg == 0) // initiate trigger pulse
			GPIO_WriteBit(GPIOB,GPIO_Pin_13, Bit_SET);

		else { // read echo pulse

			if((GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_4) != 0))
			{
					EchoCnt++; // count cycles for which echo pin is high
			}

			else
			{
				//echo pin is low, store count
				...
				EchoCnt = 0;
				Echo_WaitCount = 0;
				GPIO_WriteBit(GPIOB,GPIO_Pin_13, Bit_RESET); // end the trig pulse
			}
		}
